...
// -------------------------
  ...
  const filteredActionRows: ActionPlanRow[] = useMemo(() => {
    const rows: ActionPlanRow[] = (result?.action_plan ?? []) as ActionPlanRow[];
    return rows
      .filter((r) => (r.severity ?? 3) >= minSeverity)
      .filter((r) => (showBlockersOnly ? Boolean(r.blocker) : true))
      .slice()
      .sort((a, b) => {
        const pr = (p: string) => (p === "P0" ? 0 : p === "P1" ? 1 : p === "P2" ? 2 : 3);
        const d = pr(a.priority) - pr(b.priority);
        if (d !== 0) return d;
        return (b.severity ?? 0) - (a.severity ?? 0);
      });
  }, [result, minSeverity, showBlockersOnly]);

  return (
    <Box sx={{ p: 3, maxWidth: 1200 }}>
      <Typography variant="h4" sx={{ mb: 2 }}>
        Operational Risk & Forecast Dashboard
      </Typography>
      ...
      <ActionPlanTable rows={filteredActionRows} />
    </Box>
  );
}
